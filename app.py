...
class ResumeApp:
    ...
    #     with analyzer_tabs[0]:
    #             # Job Role Selection
    #         categories = ["Select Job Category"] + list(self.job_roles.keys())
    #         selected_category = st.selectbox(
    #             "Job Category", categories, index=0, key="standard_category")

    #         if selected_category != "Select Job Category":
    #             roles = ["Select Specific Role"] + list(self.job_roles[selected_category].keys())
    #             selected_role = st.selectbox(
    #                 "Specific Role", roles, index=0, key="standard_role")

    #             if selected_role != "Select Specific Role":
    #                 role_info = self.job_roles[selected_category][selected_role]
...
"""
    ...
    def main(self):
        ...
        # Ensure a valid page is selected in session state
        # valid_pages = [name.lower().replace(" ", "_").replace("🏠", "").replace("🔍", "").replace("📝", "")
